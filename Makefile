...
elo:
	docker build . -t clemens && docker run --rm clemens:latest /elo.sh

generate-games:
	docker build . -t clemens && mkdir -p $(PWD)/save && docker run --rm -v $(PWD)/save:/save clemens:latest /generate-games.sh
