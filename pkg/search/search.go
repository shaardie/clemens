...
func (s *Search) negamax(pos *position.Position, alpha, beta int16, depth, ply uint8, pvl *pvline.PVLine, canNull bool) (int16, error) {
	...
	if depth > 2 && canNull && !isInCheck && !pvNode && !evaluation.IsEndgame(pos) {
		ep := pos.MakeNullMove()
		v, err := s.negamax(pos, -beta, -beta+1, min(depth-3, 1), ply+1, &pvline.PVLine{}, false)
		pos.UnMakeNullMove(ep)
		if err != nil {
			return 0, err
		}
		v *= -1
		if v >= beta && !evaluation.IsCheckmateValue(v) {
			return beta, nil
		}
	}
